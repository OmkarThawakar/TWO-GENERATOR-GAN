...
def F_Measure(image,target,beta_sqr=1):
    ...
    img_dilation = cv2.dilate(x, kernel, iterations=1) 
    ...
    precision = TP/(TP+FP)
    recall = TP/(TP+FN)
    #f_measure = (((1+beta_sqr)*precision)*recall)/((beta_sqr*precision)+recall)
...
gen1 = pd.DataFrame(tmp['Generator_1'],index=video_names[:10])
gen2 = pd.DataFrame(tmp['Generator_2'],index=video_names[:10])
...
plt.plot(video_names[:],tmp['Generator_1']['SSIM'],label='Gen_1')
plt.plot(video_names[:],tmp['Generator_2']['SSIM'],label='Gen_2')
plt.legend()
plt.title('SSIM')
plt.show()
